...
Cartridge player(SPEAKER_PIN);  // make sure SPEAKER_PIN is defined somewhere

TaskHandle_t TaskHandle0 = NULL;
float GAIN = 0.1;//volume control
...
void TaskPlayMusic(void* pvParameters) {
  MusicParams* params = static_cast<MusicParams*>(pvParameters);

  esp_task_wdt_init(params->wdTimer, false);  //disable WDT for a while
  esp_task_wdt_add(NULL);
  player.play_nes(params->song, false, GAIN);  // false = no loop, gain = 5.0
  esp_task_wdt_delete(NULL);
  esp_task_wdt_deinit();
  delete params;
  vTaskDelete(NULL);  // Correct way to end the task
}

void startMusicPlayback(const uint8_t* song, int wdTimer) {

  auto* params = new MusicParams{ song, wdTimer };
  ...
  if (result != pdPASS) {
    Serial.println("Failed to create music playback task");
    delete params;
    TaskHandle0 = NULL;
  }
}
...
void Cartridge::parse_vgm(const uint8_t* music, bool looping) {
  ...
  if (vgm_wait == 0) {
    ...
    if (command == 0x61) {  // Wait nL nH samples
      ...
      uint32_t block_size = (music[vgm_index + 3]) + (music[vgm_index + 4] << 8) + (music[vgm_index + 5] << 16) + (music[vgm_index + 6] << 24);
...
void Cartridge::reset_nes() {
  ...
  //single_output = 0; // this disable output pin to speaker_pin
  // Pulse 1 Variables
  p1_output = 0;
  p1_11_bit_timer = 0;
  p1_wave_index = 0;
